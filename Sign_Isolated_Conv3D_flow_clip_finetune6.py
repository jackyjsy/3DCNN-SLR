...
# Path setting
exp_name = 'color_flow_final_pretrained_finetune6'
data_path = "/scratch/jiang.so/sign/train_val_test_flow_color"
# data_path2 = "/home/ubuntu/data/sign/train_flow_color"
data_path2 = "/scratch/jiang.so/sign/test_flow_color_2"
# label_train_path = "data/sign_train_split1.csv"
label_train_path = "data/train_val_test_labels.csv"
# label_val_path = "data/sign_val_split1.csv"
...
if __name__ == '__main__':
    ...
    model = flow_r2plus1d_18(pretrained=True, num_classes=226)
    # load pretrained

    # checkpoint = torch.load('checkpoint/swish_smooth_dropout_flow_clip/sign_resnet2d+1_epoch099.pth')
    # checkpoint = torch.load('checkpoint/swish_smooth_dropout_flow_clip_correct/sign_resnet2d+1_epoch087.pth')
    # checkpoint = torch.load('checkpoint/swish_smooth_dropout_flow_clip_correct_finetune/sign_resnet2d+1_epoch002.pth')
    # checkpoint = torch.load('pretrained/val_depth_flow_finetune2_epoch015.pth')
    # checkpoint = torch.load('final_models/val_flow_color_final_epoch_092_9165.pth')
    # checkpoint = torch.load('final_models/flow_color_finetune2_epoch30.pth')
    checkpoint = torch.load('checkpoint/color_flow_final_pretrained_finetune6/sign_resnet2d+1_epoch001.pth') 
    ...
    # Start training
    val_loss = val_epoch(model, criterion, val_loader, device, -1, logger, writer, phase='Test', exp_name=exp_name)
    if phase == 'Train':
