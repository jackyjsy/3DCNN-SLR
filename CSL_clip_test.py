...
from dataset_csl_clip import Sign_Isolated
...
# Path setting
exp_name = 'csl_rgb_clip_test'
data_path = "/scratch/jiang.so/data/CSL/images_all"

label_train_path = "/scratch/jiang.so/data/CSL/annotations/train_labels.csv"
label_val_path = "/scratch/jiang.so/data/CSL/annotations/val_labels.csv"
...
log_path = "log/csl_clip_resnet2d+1_{}_{:%Y-%m-%d_%H-%M-%S}.log".format(exp_name, datetime.now())
sum_path = "runs/csl_clip_resnet2d+1_{}_{:%Y-%m-%d_%H-%M-%S}".format(exp_name, datetime.now())
phase = 'Test'
...
if __name__ == '__main__':
    ...
    checkpoint = torch.load('checkpoint/csl_rgb_clip/epoch037.pth')
    start_epoch = 37
    ...
    if phase == 'Train':
        ...
        val_loss = val_epoch(model, criterion, val_loader, device, start_epoch, logger, writer, phase=phase, exp_name=exp_name)
