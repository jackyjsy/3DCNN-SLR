...
def val_epoch(model, criterion, dataloader, device, epoch, logger, writer, phase='Train', exp_name='exp'):
    model.eval()
    losses = []
    all_label = []
    all_pred = []
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(dataloader): 
            ...
            if phase == 'Test':
                sample_names.extend(data['video_name'])
                score_frag.append(outputs.data.cpu().numpy())
    ...
    if phase == 'Test':
        with open('./results/{}/results_epoch{:03d}_{}.pkl'.format(exp_name, epoch, validation_acc), 'wb') as f:
