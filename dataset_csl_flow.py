...
class CSL_Isolated_Flow(Dataset):
    ...
    def read_images(self, folder_path):
        ...
        if len(os.listdir(folder_path)) < self.frames:
            frame_length = len(os.listdir(folder_path)) * 2
        else:
            frame_length = len(os.listdir(folder_path))
        images = []
        start = 0
        step = int(frame_length/self.frames)
        for i in range(self.frames):
            index = (start+i*step) % len(os.listdir(folder_path)) + 1
            image = Image.open(os.path.join(folder_path, '{:06d}.jpg').format(index))  #.convert('L')
            if self.transform is not None:
                image = self.transform(image)
            # read 2 channels for flow images
            image = image[:2,:,:]
            images.append(image)
    ...
    def __getitem__(self, idx):
        ...
        video_name = selected_folder.split('/')[-1]
        ...
        return {'data': images, 'label': label, 'video_name': video_name}
